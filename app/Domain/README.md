...
| `Inventory` | Unified inventory (ADR-0012): categories/items/variants, stock movements, purchase orders, equipment assignments, supply requests. Issuing a uniform *starts* a charge schedule, which lives in `Adjustments` |
| `Adjustments` | Everything deducted from or added to contractor pay. Immediate incentives/deductions (`adjustment_items`, `time_entry_adjustments`; billable incentives flow to invoices, deductions are payroll-only) **and** amounts collected over several pay periods (`contractor_charge_schedules` + entries, ADR-0014) — the hiring fee charged on promotion, and uniform charges started by Inventory. `AllocateChargeScheduleEntries` extends a schedule as payroll periods open; `ApplyScheduledContractorCharges` turns each due entry into a deduction |
